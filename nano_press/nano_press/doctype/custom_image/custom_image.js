		...
		callback: function (r) {
			if (r.message && r.message.success) {
				...
				const dialog = new frappe.ui.Dialog({
					...
					size: "large",
					fields: [
						{ fieldtype: "HTML", fieldname: "apps_summary", label: __("Apps Summary") },
						{ fieldtype: "Code", fieldname: "apps_json", label: __("Generated apps.json"), options: "JSON", read_only: 1 },
						{ fieldtype: "Small Text", fieldname: "base64_preview", label: __("Base64 (first 200 chars)"), read_only: 1 },
					],
				});
				...
				dialog.set_value("base64_preview", data.apps_json_base64.substring(0, 200) + "...");
				dialog.show();
			} else {
				frappe.msgprint({

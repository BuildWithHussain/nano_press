	...
	refresh(frm) {
		...
		if (!frm.is_new()) {
			...
			if (frm.doc.verify_status === "Prepared" && !frm.doc.traefik_deployed) {
				frm.add_custom_button("Deploy Traefik", () => {
					// Validate required fields before calling
					if (!frm.doc.traefik_domain || !frm.doc.traefik_email ||
						!frm.doc.traefik_username || !frm.doc.traefik_password) {
						frappe.msgprint({
							title: __('Missing Information'),
							indicator: 'orange',
							message: __('Please fill in all Traefik fields: Domain, Email, Username, and Password')
						});
						return;
					}

					frappe.call({
						method: "nano_press.nano_press.doctype.server.server.deploy_traefik",
						args: {
		...
		if (frm.doc.verify_status === "Verified" || frm.doc.verify_status === "Prepared") {
			// Only show Prepare when verified
			frm.add_custom_button("Prepare Server", () => {
				...
				if (frm.doc.verify_status === "Prepared") {
					frappe.confirm(
						...
						() => {
							...
							frappe.call({
								...
								callback: (r) => {
									...
									if (r && r.message) {
										if (r.message.status === 200) {
											frappe.show_alert({ message: "Server prepared successfully", indicator: "green" });
										} else {
											frappe.show_alert({ message: "Server preparation failed", indicator: "red" });
										}
										frm.reload_doc();
									}
								},
						...
						() => {
							...
							frappe.show_alert({ message: "Preparation cancelled", indicator: "orange" });
						}
					);
				} else {
						...
						callback: (r) => {
							if (r && r.message) {
								if (r.message.status == "success") {
									frappe.show_alert({ message: "Server verified", indicator: "green" });
								} else {
									frm.set_value("verify_status", "Failed");
									frappe.show_alert({ message: "Verification failed", indicator: "red" });
								}
								frm.reload_doc();
							}
						},
		...
		frappe.call({
			...
			callback: (r) => {
				if (r && r.message && frm.fields_dict.public_key) {
					...
					btn &&
						btn.on("click", async () => {
							const text = frm.fields_dict.public_key.$wrapper.find("#server-public-key").text();
							try {
							...
							frappe.show_alert({ message: "Public key copied", indicator: "green" });
						});
				}
			},
		});
